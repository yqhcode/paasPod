...
import (
	...
	ratelimiter "github.com/asim/go-micro/plugins/wrapper/ratelimiter/uber/v3"
	...
	service2 "github.com/yqhcode/paasPod/domain/service"
	"github.com/yqhcode/paasPod/handler"
	hystrix2 "github.com/yqhcode/paasPod/plugin/hystrix"
	"github.com/yqhcode/paasPod/proto"
)

var (
	//服务地址
	hostIp = "192.168.65.146"
	//服务地址
	serviceHost = hostIp
	//服务端口
	servicePort = "8081"
...
func main() {
	...
	//初始化数据库
	port := strconv.FormatInt(mysqlConfig.Port, 10)
	db, err := gorm.Open("mysql", mysqlConfig.User+":"+mysqlConfig.Pwd+"@("+mysqlConfig.Host+":"+port+")/"+mysqlConfig.Database+"?charset=utf8&parseTime=True&loc=Local")
	if err != nil {
		fmt.Println(err)
		common.Error(err)
	}
	...
	//添加监听程序
	go func() {
		err2 := http.ListenAndServe(net.JoinHostPort("0.0.0.0", strconv.Itoa(hystrixPort)), hystrixStreamHandler)
		if err2 != nil {
			common.Error(err2)
		}
	}()
	...
	//注册句柄
	podService := service2.NewPodService(repository.NewPodRepository(db), clientset)
	proto.RegisterPodServer(service.Server(), &handler.PodHandler{PodService: podService})
